...
sagaGIS <- function(saga_bin = NULL,
  ...
  # store miscillaneous options
  senv[["saga_options"]] <- list(
    shape_ext = ifelse(senv$saga_version >= numeric_version("7.0.0"),
                      ".gpkg", ".shp")
  )
...
sagaExecute <- function(lib, tool, senv, intern = TRUE, ...) {
  ...
  saga_version <- senv$saga_version
  saga_options <- senv$saga_options
  ...
  for (i in seq_along(args)) {
    ...
    if (any(class(args[[i]]) == "list")) {
      for (j in seq_along(args[[i]]))
        args[[i]][[j]] <- RtoSAGA(args[[i]][[j]], senv$saga_options$shape_ext)
    } else {
  ...
  if (n_temps > 0) {
    ...
    for (i in 1:nrow(unspec_out)) {
      if (unspec_out[i, "Feature"] %in% c("Grid", "Grid list", "Raster")) {
        ...
        tfile <- tempfile(fileext = saga_options$shape_ext)
        unspec_out[i, "args"] <- tfile
...
#'   object or dataframe
#' @param ext character, file extension to use for vector data
#' For SAGA versions < 7.0.0, this should be '.shp', while for newer
#' SAGA versions the geopackage format ('gpkg') is used to preserve field
#' names longer than 7 characters
#'
#' @return character. Character string of filename of saved object
RtoSAGA <- function(param, ext) {
  sf_to_saga <- function(param, ext) {
  ...
  shape_to_saga <- function(param, ext) {
    temp <- tempfile(fileext = ext)
    pkg.env$sagaTmpFiles <- append(pkg.env$sagaTmpFiles, temp)
    rgdal::writeOGR(
      obj = param,
